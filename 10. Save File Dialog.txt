...
#define FILE_SUB_MENU_SAVE_DIALOG 7
	...
	switch (msg) {
		case WM_COMMAND:
			...
			case FILE_SUB_MENU_OPEN_DIALOG:
				...
				openFileDialog(hWnd);
				break;
			case FILE_SUB_MENU_SAVE_DIALOG:
				EnableWindow(hWnd, true);
				saveFileDialog(hWnd);
				break;
			}
...
void saveFileDialog(HWND hWnd) {
	...
	// Use file_name to assign file name
	saveFileName.lpstrFile = file_name;
	saveFileName.lpstrFile[0] = '\0';

	saveFileName.nMaxFile = 100;
	saveFileName.lpstrFilter = "All File\0*.*\0Source File\0*.cpp\0TEXT Files\0*.txt\0";
	saveFileName.nFilterIndex = 1;

	GetSaveFileName(&saveFileName);
	writeFileContent(saveFileName.lpstrFile);
}

void writeFileContent(char* path) {
	FILE *f;
	errno_t err;
	...
	if (err == 0)
	{
		int _size = GetWindowTextLength(hEditDialog);
		char *data = new char[_size + 1];
		GetWindowText(hEditDialog, data, _size + 1);

		fwrite(data, _size + 1, 1, f);
		fclose(f);

	}
	else
	{
		printf_s("This file %s didn't save", *path);
	}
}
...
void displayDialogBox(HWND hWnd) {
	...
	// (HMENU): Tạo ra sự kiện cho các category
	CreateWindowW(L"button", L"Save File", WS_VISIBLE | WS_CHILD, 170, 20, 100, 40,
		hWnd1, (HMENU)FILE_SUB_MENU_SAVE_DIALOG, NULL, NULL);

	hEditDialog = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | BS_MULTILINE | WS_BORDER | WS_VSCROLL | WS_HSCROLL,
